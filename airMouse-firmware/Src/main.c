...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	sensorStat = lsm6ds_setGROutputDataRate(&mems, LSM6DS_ODR_G_26_HZ);

	sensorStat = lsm6ds_setXLFullScale(&mems, LSM6DS_FS_XL_16G);
	sensorStat = lsm6ds_setGRFullScale(&mems, LSM6DS_FS_G_245DPS);



	int32_t sensitivityDivider = 5;
	int32_t accelerationDivider = 10000;
...
//		ledOff();
		...
		if (newDataAvailable & LSM6DS_GDA) {
			...
			amx = (int32_t)mems.outGR.x / sensitivityDivider;
			amz = (int32_t)mems.outGR.z / sensitivityDivider;
			...
			amx /= accelerationDivider;
			amz /= accelerationDivider;
			...
			if (HAL_GPIO_ReadPin(BUTTON_BLUE_GPIO_Port, BUTTON_BLUE_Pin)
					== GPIO_PIN_RESET) {
				espAt_sendParams(&bleRadio, P_BHM, 4, 1, amz * -1, amx * -1, 0);
			}else{
